...
| 层 | 职责 |
|---|---|
| 记分牌 | 辩题/进度/双方身份与模型徽章（全页仅此一处常驻）/布局开关 |
| 剧本主列 | 正反：轮次发言+质询。旧场红队/圆桌仍按原形态回放。发言**无**模型徽章 |
| 终审舞台 | 唯一结论面：裁决倾向（胜负手 + 置信档）+ 交接清单。正反不展最强论点；有交接不展建议 |
...
辩论进行中：主输入框对这场说话（`/debate-steer` continue，下一轮生效）；「出结论」= conclude。窗关诚实拒收。收场后再辩 = 对 CEO 说话重开。站队已删。