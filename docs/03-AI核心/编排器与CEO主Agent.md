...
CEO 是**管理者**，也是判断者：要不要拉人由他判（像主程序员决定要不要开子代理）。主要持只读 / 检索工具。闲聊 / 窗口已有证据则自己开口；判断若取决于工作区文件则先取证（取证≠必须开组）；生产 / 变更一律 `delegate`（档 2.5：CEO 无写权）。成规模取证按自然缝编制（能 1 人则 1 人，真并行才多人），禁止自己连搜收齐结论；人数不按话题类型套默认。已装配 `terminal` 时，可对工作区长驻进程做启/停/读（纯启服轻量例外；云桌 guest / 本机桌面）。

底线：对用户呈现**一个 CEO 声音**；轻量闲聊 / 单点只读直答与纯启服（零编排开销）；组团 / 动手 / 成规模取证按需触发。
...
**探路 vs 摸底**（提示词纪律，不是引擎硬闸）：探路只回答「从哪几个入口进」，默认 0～1 轮。判断 / 方案若取决于工作区现行文或代码、窗口里还没有 → **先取证再开口**；用户说「讨论」不改变这条。取证可以是自己读几份相关文件，不等于必须开组，也不等于自己摸完整仓。成规模取证 = 摸底：禁止自己搜完再整理，编制按自然缝自选（能 1 人则 1 人，真并行才多人）；禁止因话题像盘点 / 架构就默认 ≥2。**派工先判要不要**（必须=改产物 / 应该=成规模取证），**再判拉几人**（真并行才多人）；判断不因「未落盘」而必须派。**组队靠提示词（CEO 自判）**；引擎不剥调查工具、不丢闸后长文。探路纪律仍是 0～1 轮；开口前读相关文件是取证，不是接着自己横扫。引擎不扫用户原文猜意图。
...
**档位取舍**：档 2.5 = 结构取档 2（CEO 只读 + 窄例外；否决档 1 全能 CEO、档 3 纯编排 CEO）+ 路由由 CEO 自判（先取证再开口；改产物必须派；成规模取证应该派；真并行才多人）。档 1 污染上下文、弱化团队心智；档 3 给高频轻量只读 / 纯启服加委派税。
...
| **委派** | **必须**（工具边界）：实质交付物（代码 / 应用 / 要求落盘的成篇文字，哪怕一行）——CEO 无写权；**应该**（规模）：成规模取证（横扫多来源、自己会连搜收齐）。判断不因「未落盘」而必须派。几人见下行 |
...
短文分界：未要求存文件的短文 / 短改写、以及窗口里已有证据的判断 → 回复直写；缺工作区证据 → 先取证；成规模取证才派；明确落盘 → 派 1 人。CEO 绝不为省委派把整份代码贴进正文。
