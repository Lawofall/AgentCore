...
仅提示词 / Skill（拦截阶梯 1；→ 见代码: `runtime/resolve/prompt/` · `ask_user_kickoff`）；**禁**硬闸、**禁**意图分类器扫文猜「该不该换载体」、禁复活场面硬账（权威 → [检查点 · §一](/docs/03-AI核心/检查点与开工卡.md)）。提示词只写可复用原则，**禁**为单次失败 case 写话术剧本（具体回归放 conformance）。
...
| `code_audit` | 五章 + `*.audit.json` 结构闸；名的是发现台账，不泛化成任意 ledger | 留。台账合同就是活 |
| `map_fanout` | N 路并行一页地图、无合成/审校节点；防误升成文专线 | 留。价值在减法 |
| `diagnose_fix_verify` | 无先验调查批的单症状两波（修补含短诊断→独立验证）；`verify` 必填 | 留合同。已定位文件 / 已有调查批 → **勿套本**，手写（可 1 人，verify 写进 task） |
| `lens_crosscheck` | 异质透镜交叉核验 + `motion_card`；`lenses` 必填 ≥2 | 留但窄。仅公共事件 / 品牌危机等须分开查的异质透镜 |

**已删**：`build_app`（Vue/SPA 工厂图纸，不是「做软件」；真实开发会覆盖用户方案）；`compare_options`（与手写「点名对比 ≥N 人」同拓扑）；`build_feature`（前后端并行 = 技能「契约共享面」，且 `include[]` 分叉）；更早的 `build_website` / `build_website_verify`（建站手写一人整页）。共享口径只走非空 `team_brief` 或短规格岗，不因具名本另开边信道。

**现行钉死**：上表五本全留；`debate` 不进 `PLAYBOOKS`。加本仍过本节准入。
