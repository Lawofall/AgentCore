...
**治理靠可见性**：配额停摆仍推 `kind=quota` 卡（**什么没写进来、谁占着池子**）——这是取代「权威分档」的机制，AI 无法自我提权。闲聊收尾、探索、`remember` **不**发 `kind=semantic` 巩固卡；常驻写进了什么，看文件页，不在对话尾等一张「写进去了什么」。存量 `semantic` 行仍可读，与 `quota` 共用同一套行组件（`MemoryUpdateItemRow`），故工具箱提示词流水账对旧卡仍同一读法。情景沉淀只落 `memory_episodes`，**不出卡**。纠错通道入口是文件页「这条不对」，不依赖对话尾巩固卡。
...
查看/编辑：对话内 `remember`（增改删列）与工具箱提示词（流水账 `/toolbox/mine/skills?updates=1`）+ 各文件夹 ``.agentcore``、右坞工作区同一 ``.agentcore``（常驻槽位摊开；主题默认折在「主题」夹）+ CAS 双轨；semantic diff 可搬层纠错。→ 见代码：`fileWorkbench/AgentCoreSection.tsx` · `EntriesSection.tsx` · `workspace/WorkspacePanel.tsx`
...
条目化后文件页不再有「记忆 / 规则 / 文档」三夹：账号共用提示词在工具箱（流水账 `/toolbox/mine/skills?updates=1`）；各文件夹条目与过程稿合在 ``.agentcore`` 里（默认折叠；打开后根上是常驻、`主题/*.md` 收进默认折叠的「主题」夹（按需），行上不标常驻/按需徽章，仍可带 `description`，稿夹摊平为 `工作稿`/`research`/`debate`/`reviews`）。「新建条目」在 ``.agentcore`` 标题行右侧（折叠时也能建）。AI 维护的条目右键含**「这条不对…」**（先弹确认摊开连带面）；已停用的（含手写存量）划掉、标「已停用」、可「恢复使用」，也不再报它的常驻占用（它已不占池）。手写条目不要了就删。核心叶右键是**「清空」**（不是「删除」）。
