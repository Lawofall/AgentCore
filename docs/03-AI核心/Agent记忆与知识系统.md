...
- **句子级：搬走，不是标记**：让用户只否他看见的那一句。主入口是文件页「这条不对」；存量 `kind=semantic` 对话尾卡每行仍可一键搬走（新巩固卡不再发）。不弹确认，toast 带撤销；过后从「记忆动态 → 已移走的记忆」放回。→ `memory/dispute_line.py`
  ...
  - **入口必须把改动喊出去**：文件页、存量对话尾卡、找回面「记忆动态」各有缓存——在一处否掉一行却不让另一处失效，用户去找就是「什么都没发生」。故行组件的回调（`onMemoryChanged`）是每个宿主的义务，不是可选装饰。
...
**治理靠可见性**：配额停摆仍推 `kind=quota` 卡（**什么没写进来、谁占着池子**）——这是取代「权威分档」的机制，AI 无法自我提权。闲聊收尾、探索、复盘勾选、`remember` **不**发 `kind=semantic` 巩固卡；常驻写进了什么，看文件页与每日复盘勾选，不在对话尾等一张「写进去了什么」。存量 `semantic` 行仍可读，与 `quota` 共用同一套行组件（`MemoryUpdateItemRow`），故文件页「最近更新」对旧卡仍同一读法。情景沉淀只落 `memory_episodes`，**不出卡**。纠错通道入口是文件页「这条不对」，不依赖对话尾巩固卡。
...
**「画像 / 偏好默认常驻」不需要运行时守卫**：生效档是条目自身的属性，而 AI 写入常驻（探索 / 复盘 / 文件页归并）的主要动作是**把新事实归并进已有条目**——**归并不改目标的生效档**。`偏好.md` / `画像.md` / `导航.md` 是常驻，`主题/*.md` 是按需；没有路径能让画像悄悄滑成按需（除非用户自己改）。真正要判生效档的只有**新建**条目那一刻，且判错的代价是「新事实进错了地方」而非「核心画像失效」（旧条目仍在、仍常驻），这个量级交提示词判据即可（几乎每个任务都要用的身份 / 合作方式 → 常驻；某领域厚知识 → 按需），符合 `intercept-discipline` 阶梯 1，不上闸。否决表「偏好/画像改 on_demand」仍成立。
...
⏳ **余项**：取消 `role` 三分；探索 / 复盘按 `description` 归位（现状探索仍整文件重写文件夹画像；**不得预置空条目**）。用户技能入基座 ✅（系统 Skill 真源留代码；占槽 overlay 权威 → [工具与能力 · 用户技能](/docs/03-AI核心/工具与能力系统.md#用户技能与可编目录)）。
...
> 闲聊收尾**不写常驻**。情景摘要落 `memory_episodes`、立即标已消化、**不注入**、不出卡。常驻（`偏好.md` / `画像.md` / `导航.md`）只由 `remember`、探索幕、每日复盘勾选、文件页写入。✅ **主题不由闲聊写**。⏳ 探索 / 复盘仍是整文件或按节合并，目标形态按 `description` 归位；**不得预置空条目**。
...
| 写到哪 | 记什么 | 够格的证据 | 谁写 |
|---|---|---|---|
| 偏好.md | 怎么跟我合作 | 用户明示或当场纠正；几乎每个任务都要用 | `remember` 不写此叶（只写用户规则）；每日复盘勾选 · 文件页 |
| 画像.md | 我是谁 / 这桌是什么 | 用户陈述；几乎每个任务（或这张桌上每个任务）都要用；文件夹层另加探索摸仓 | 探索幕 · 复盘勾选 · 文件页 |
| 导航.md | 下次能省掉的一步 | 动作清单实证 | 探索幕首建；旁路刷新增量 |
| 主题/*.md | 以后还会来查的厚知识 | 不能从产品或工作区当场拿到 | 探索幕（含旁路刷新）· 文件页 · 每日复盘勾选。`remember` 仍只写用户规则 |
...
**系统模板 · 每日对话复盘** ✅：站立任务 `template_key=daily_conversation_review`（引导开、默认日跑）。作用域可配（全局裸聊 / 多个云文件夹 / 回看小时）。**无新料硬闸**：作用域内无近期对话则收件箱直接「今日无新料」、不跑 LLM。有料时代跑 brief 要求 `ask_user card=daily_review`；用户勾选确认后**服务端直接**写记忆 / 用户规则 / `AgentCore/文档/reviews/`（不再依赖 LLM 再调 remember）。勾选可写主题与常驻——闲聊收尾不写常驻之后，这是人审入口。preference/profile 只应勾「几乎每个以后任务都会用到」的；一次性过程进 topic 或 doc。→ `standing_tasks/templates.py` · `review_apply.py` · `review_preflight.py`；桌面 Toolbox → 自动化。
...
| 闲聊摘要自动写入常驻（偏好/画像/导航） | 一场任务渣进每回合上下文；常驻只留几乎每个任务都有用的，入口是 remember / 探索 / 复盘勾选 / 文件页 |
...
| 扫用户原文猜「是不是在问功能」再跳过沉淀 | 意图分类器；正确滤网是闲聊不写常驻、主题不在闲聊出口，不是给对话分类 |
...
**核心叶可清空、槽位保留**：`画像.md` / `偏好.md` / `导航.md` 的**正文**可经文件页右键「清空」去掉（危险确认；走已有 `PUT /v1/users/me/memory/files/{kind}` 空正文，与编辑器抹空同一条）。空了就不注入。列表仍占位（与冷启动占位同一套）。`DELETE /v1/documents/{id}` 仍拒核心叶——固定文件名是加载协议，不是第三条内容类型。探索 / 复盘勾选 / `remember`（规则）之后仍可能再写入；拦复活是另步。
