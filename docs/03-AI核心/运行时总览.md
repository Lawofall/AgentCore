...
2. **组团 · delegate / debate**：CEO 主要持只读；改产物走 `delegate`；活明显超出一轮短答（成规模查证、实质讨论、对照多方案）也交给团队。闲聊和窗口里已有的短答自己回。编制按自然缝，真并行才多人。`depends_on` 边定形 DAG（无依赖即并行），是「串行 / 并行 / 混合」的唯一开关。用户点名开辩才调 `debate`（日常多视角走组队）。组队 / 点名后的开辩**直接开跑**。会话 **PermissionAxes** 管写盘 / 执行 / 本机 Host，不再含组队确认轴。→ [编排器 §一·delegate / §五](/docs/03-AI核心/编排器与CEO主Agent.md) · [安全权限与治理 §三](/docs/05-平台与运维/安全权限与治理.md)
