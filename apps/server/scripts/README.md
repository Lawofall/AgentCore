...
- **运维 / 开发**：`create_admin.py`、`seed_dev_user.py`、`set_quota.py`（同构档：`set_quota.py dev --unlimited`）、`set_dev_llm_key.py`、`export_conversations.py`、`cleanup_test_conversations.py`、`backfill_memory.py`、`migrate_memory_pipeline.py`、`backfill_auto_desk_scratch.py`、`sync_community_prices.py`、`fetch_*.py`、`start-dev-server.ps1`
- **演示带**：`demo_tape_*.py`
