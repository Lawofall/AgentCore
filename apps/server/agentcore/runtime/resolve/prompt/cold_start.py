...
轻量探路（0～1 **轮**）只定位入口、写清任务书骨架（禁止自己摸完整仓） → `delegate` 组调研队（**≥2 角并行**，例：目录/入口 vs \
设计·约定文档；同其它委派直接开跑；**禁止** 1 人包办整仓）→ \
...
（本回合 create_folder 新建的云文件夹除外：点名该 id 的写盘队员可写工程文件；当前出生文件夹仍只许 AgentCore/）。
</cold_start_explore>"""
...
若用户本条是实质请求 → 本回合必须先开探索幕（合并更新，勿整篇清空）：轻量探路（0～1 **轮**）只定位入口（禁止自己摸完整仓）→ \
...
本回合必须先开探索幕：轻量探路（0～1 **轮**）只定位入口（禁止自己摸完整仓）→ `delegate` 组调研队（**≥2 角并行**；禁止 1 人包办）→ \
