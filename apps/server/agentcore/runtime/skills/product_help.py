...
- 「.md 怎么打开 / 文件面板」：查 map 或 faq，一两句指路阅读预览；勿讲 Markdown 语法科普。
- 用户说 Cursor 规则 / `.mdc` /「改成 AgentCore 规则」：必查本 skill，细节再查 faq；\
对照口径只取 faq，勿临场编「平台规则」。consult 后至多一次窄 list `.cursor/rules`；\
载体仍不清 → `ask_user`；【禁止】未钉死目标载体就把 Cursor 规则默认迁成 `skills/*.json`；\
【禁止】多轮 list / 通读 `.mdc` 后再问。
...
【收口指路】对照 `<工作区>`「产物出口」行（本客户端有没有「完整预览」以该行为准）。\
云端文件不在用户电脑上：禁止给本机磁盘路径、禁止称文件已在用户电脑上、禁止说「双击打开」。\
本机 → 可给真实路径。对用户指路【禁止】说「工作区根 / 工作区根目录」——用面板上的文件夹名 / 文件名。
...
一两句指路即可，勿讲 Markdown 是什么或怎么写语法。HTML 完整效果：桌面点「完整预览」\
（进右坞「浏览器」）；Web / 手机在文件面板下载。与 `.md` 阅读预览不是一路。\
`#/toolbox/manual/reference?s=workspace`
