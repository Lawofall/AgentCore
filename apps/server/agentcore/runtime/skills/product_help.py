...
手机「我的 → 模型配置」（无独立服务商页，接入 Key 与配组合同页）/「我的 → 用量」。\
...
- 工具箱 → 能力图鉴：工具与提示词清单
- 设置：模型与偏好等。桌面侧栏（模型 / 服务商 / 用量 / 外观 / 快捷键 / 反馈 / 关于）→ `#/toolbox/manual/reference?s=settings`；\
手机底栏「我的」（模型配置 / 用量 / 账户设置 / 新会话默认权限 / 关于）。\
手机无独立服务商页（并进模型配置）；无外观、快捷键、反馈入口。
...
- 用什么模型？——到 https://jiurelay.com/ 免费自行配额度后接入；也可自带 Key（BYOK）。\
桌面：接入在「设置 · 服务商」、组合在「设置 · 模型」；手机：都在「我的 → 模型配置」。`?s=faq`
...
- Key 报错？——核对 Key / 地址 / 模型名（桌面「设置 · 服务商」，手机「我的 → 模型配置」）；\
可换一家服务商或自带 Key 再试。\
...
手机没有应用内反馈入口（有意如此，非缺失），据实说明，**勿指向不存在的页面**（手机「我的」下只有模型配置 / 用量 / 账户设置 / 新会话默认权限 / 关于）。\
可提示把上方 L2 要点粘进描述。\
本档不加提交工具、不改反馈 API。
