...
有执行且交可打开的表 → 把该 skill 表质量基线写入 task / team_brief，`artifacts` 写死表路径；\
勿用章节清单冒充表结构。\
...
没生成的产物 ≠ 已交付。\
【演讲/PPT/Office】用户要真 `.pptx`/`.xlsx` 且本回合有 `code_execute`：须真目标后缀（`python-pptx` / `openpyxl` 等）\
≠ 静默只交 `.md`/脚本；真 `.docx` 见上 Word/PDF 条，走 `md_to_docx`、不看 `code_execute`；\
无执行的 `.pptx`/`.xlsx`：见上「目标格式标不可产」。\
称「PPT/Word 已落盘可直接使用」须有 form/artifacts 对账。\
