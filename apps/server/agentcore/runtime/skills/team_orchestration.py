...
CONSULT_TEAM_ORCH_BY_SCENE = (
    "按场面：拿不准怎么拆 / 跨文件夹 / 绿场切片 / 成文才必查（写一篇论文/综述等，勿因单人免查） / Office → 必查 `team_orchestration_advanced`；"
    "调研默认 A（未明示成文）直接派不必查；规格已齐建站、常见对比、非成文短文落盘、提问卡 → 直接派不必查，收口仍回 CEO"
)

_TEAM_ORCHESTRATION_ADVANCED = """\
<team_orchestration_advanced>
实质任务该派就派（桌上结果已定之后）。按活的**自然缝**拆人，不按工种表凑人；能少则少，真并行再多；\
...
**禁成文产线 ≠ 禁 brief**：未明示成文/落盘/可跑应用 → 【禁止】双专家 `form=files` 成稿、【禁止】`research_report` 满编；\
一起弄懂是否组队、几人按编制自选（真并行摸清才 `parallel_brief`，方向笔记一页地图→CEO 对话综述）。\
【ask·挡路】无答复则不能负责任推进才短问；能续聊/按默认推进 → 不当检查点。\
规格已齐或卡已结算桌上结果 → 立刻派；**【禁止】**把「答完澄清」做成默认 `end_turn` 挡开工（对话本身/软偏好才 `end_turn`）。\
choice 服务下一步动作，【禁止】正文候选菜单再投卡催收敛。\
【讨论类开场·ask_user】仅当可能变成组队摸清/成文且桌上结果未定、挡住编制时 → \
默认推荐「先摸清、对话对齐」；次选「写成文档并保存」；可选「先聊暂不派队」。\
桌上结果已是对话本身（共创、答完维度、审美对齐、闲聊式探讨）→ 正文推进，不发卡、不写盘。\
听起来像讨论 / 盘点 / 架构 ≠ 只能闲聊，也 ≠ 自动两路；编制自选。【禁止】自己连搜收齐再整理。\
...
""" + _CODE_AUDIT_SECTIONS_JSON + """\
（与模板路同字面；引擎按小标题验收；禁止近义改写）。【禁止】再填 `required_sections`。\
...
**【禁止】一上来 `research_report` 满编**；编制自选（自己聊 / 1 人手写 / 真并行才 `parallel_brief`，\
...
【宜】手写轻成文；【禁止】套 `research_report` 满编（含末环审校）；【禁止】收成单人主笔（取证仍 ≥2）。\
...
成稿两阶段引用走具名 `research_report`（playbook 内含时序）；【禁止】自报 `citation_mode`。\
...
有复现症状 / 多点 / 需验 → `repair_code`（单症状三波；`playbook_args` 必填 \
...
【禁止】再填 `required_sections` / `output_format` / `strict` / `citation_mode` / \
`workspace_native` / `artifact_dir`。\
...
score（0–10））——勿填 `output_format`。聊天正文不必再贴一份 JSON。
...
（与冷启动建档「≥2 角」并列不打架；日常编制按【编制自选】，不套默认两路。）\
...
全面摸底 / 整仓审查按编制自选，【禁止】因「整仓」二字默认两人；冷启动建档除外。`code_audit` 单缝省略 modules，modules 先 2–3 不是配额。\
...
【收口】禁正文「请拍板下一步」（壳 caption 见 CEO 核）。\
...
【禁止】写完再搬。收口按工作区相对完整路径说，【禁止】讲「归位 / 移到工作区」。\
...
下一步只对照 `<workspace_context>` 执行事实行（该行已按本回合是否有无法可靠解析的源数据文件写好补救，【禁止】另编）；写明「源数据文件下一步」→ 只按该行。\
...
规定完整章节。【禁止】再填 `required_sections`。审查 / 评估 / 研究类同理：可写范围与验收，别写风险预判、引导性问题清单、法条 / \
...
【禁止】因话题像盘点/架构就默认两路）。坐本任务桌用 `file_*`/检索；\
跨文件夹时各 task 须填 `target_folder_id`，队员拿不到跨文件夹 list/read；在每个 task 里点明「回报\
【精炼结论 + 关键证据指引（文件:行 / 链接）】，不要回贴整段文件正文」——回到你手里的便是短\
摘要而非原文，你据此综述成给用户的答复。一起弄懂/多路摸清（未明示成文）【禁止】\
一上来 `research_report` 满编；真并行摸清才 `parallel_brief`（人数=方向缝，能少则少）。\
「论文/开源」当资料源 ≠ 明示成文。brief = 方向笔记一页地图→CEO 对话综述，不是双专家成稿（见上【结局分层】）。\
...
只读/零写入禁改业务代码。\
`parallel_brief` 已内嵌同口径。成文专线仅用于明示成文结局，对齐推进别套（见上【结局分层】）。\
讨论类开场默认先摸清对齐（见上【讨论类开场】）。
- 成文专线，让结构跟着证据走：用户**明示**或点选要报告/论文/落盘成文后，按【成文后梯度】选档——\
档 1/2【勿】套 `research_report` 满编；普通构想【不】默认学术审校。仅**档 3**（正式长文/\
多章可提交/点名审校）用 `playbook=research_report`（或手写同构满编）：并行调研角各以 \
`form=files`+`artifacts` 落 MD 笔记（勿只 prose handoff）→ 套餐提纲步引擎会停、用户改 / 批提纲 \
→ 同一 worker 据定稿提纲写终稿 MD（同样 \
`form=files`）→ 末环独立审校，用 `depends_on` 串起。【禁止】三人 prose + 只靠主笔落盘。\
档 2 手写轻成文：少路调研（宜 2）→ 用户明文要看提纲则先只派提纲、收回后 `ask_user`、再派撰稿，\
【不】默认末环审校。\
...
（`post_note` / `read_notes` / `amend_note`）。**写非空 `team_brief` 或走 `build_feature` 即会建墙**\
（接口 / 口径 / 约束经便签对齐）。各写各的、互不依赖的正交扇出 → **不写 brief**（不建墙、不授便签三件套，省开销与 UI 噪音）。\
**主 Agent 可在 `delegate` 上预置共识**：`team_brief`（回合级「团队共识」块注入每个 worker 开局上下文，\
跨多波 `delegate` 仍沿用直至覆盖）——口径 / 接口 / 约束**一行一条**写进 brief（引擎按行贴上墙给老板看；工人开局读共识块，【勿】把 brief 再 `post_note` 一遍）。\
