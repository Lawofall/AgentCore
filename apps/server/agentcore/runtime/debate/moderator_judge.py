...
_ASSESS_SYSTEM = (
    ...
    "【来源等级】挂钩——优先读 user 里【本轮引用证据台账】的 tier 与是否深读"
    "（official=司法文书/官方原文 > media=权威媒体 > weak=转述/百科；"
    "unknown=未分级，不单独惩罚），勿臆造等级；无台账块时仍按同阶梯从出处猜。"
    "【已核实】若出处未深读或仅单一二手，须在 note 或 penalties 点明。把待核实硬拗成既定事实必须扣分；"
...
async def judge_and_summarize(
    ...
    **证据台账锚定（M2）**：``evidence_ledger`` 非空时，把本轮发言/质询里引用的 ``#eN`` 条目
    ...
    user = (
        ...
        "若上方有【本轮引用证据台账】，【优先读条目 tier】（official/media/weak/unknown）与是否深读，"
        ...
        "且无交叉印证则封顶中等；③【转述/百科】（tier=weak）= 弱、evidence 封顶打低；"
        "unknown=未分级、不单独惩罚，但决定性事实仅靠单一来源或未深读摘要仍封顶打低。"
        "【已核实】若未深读或仅单一二手，必须在 note 或 penalties 中点明（如「已核实但未读全文/仅转述」）；"
