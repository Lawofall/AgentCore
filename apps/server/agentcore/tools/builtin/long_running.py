...
def effective_wait_for(command: str, wait_for: object) -> str:
    """Ready regex only when the caller set one; omit means return after spawn."""
    del command
    return str(wait_for or "").strip()
...
def long_running_redirect_message(
    ...
    return (
        ...
        "设 background=true，填入同一命令（省略 wait_for 则起来就返回）；"
