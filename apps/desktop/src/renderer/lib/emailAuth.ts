...
export function isGeneratedHandle(
  ...
  return u.length > 0 && d === u && GENERATED_HANDLE_RE.test(u);
}

/** Whether the public handle is still the server-allocated `user_*` token. */
...
export function usernameFieldError(value: string): string | null {
  ...
  if (trimmed.includes("@")) return "用户名不能包含 @";
  const lower = trimmed.toLowerCase();
  if (lower.startsWith("user_")) return "用户名不能以 user_ 开头";
  ...
  if (RESERVED_USERNAMES.has(lower)) return "该用户名不可用";
  return null;
}
